function polys = intersectPlaneMeshYDW(plane, v, f, e, edges)
...
for iEdge = 1:length(edgeCrossInds)
    ...
    id1=f==edge(1) | f==edge(2);
    indFaces = find(sum(id1, 2) == 2);
%     indFaces2 = find(sum(ismember(f, edge), 2) == 2);
%     assert(sum(indFaces2==indFaces)==2)
